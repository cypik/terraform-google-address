...
locals {
  ...
  regional_addresses_count = var.global ? 0 : length(var.name)
  global_addresses_count   = var.global ? length(var.name) : 0
...
resource "google_compute_address" "ip" {
  count        = local.regional_addresses_count > 0 && var.ip_count > 0 ? var.ip_count : 0
...
resource "google_compute_global_address" "global_ip" {
  ...
  project       = data.google_client_config.current.project
  name          = format("%s", module.labels.id[count.index])
  address_type  = var.address_type
  address       = element(var.addresses, count.index)
